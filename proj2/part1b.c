...
int *iterative_spawn(int kids, int data_length, int * vals){
    ...
    if(sum_c == kids && min_c==kids &&max_c==kids){
        for(int i = 0;i<kids;i++){
            ...
            printf("your shit fucked up %d kids with %d sums, %d mins, %d maxs\n",kids, sum_c,min_c,max_c);
    }
...
int main(int argc, char *argv[]){
    ...
    sigaction(SIGRTMIN+7, &act2, NULL);
    ...
    //sigaddset(&mask2, SIGRTMIN+7);   // for marking children

    //sigaddset(&mask3, SIGRTMIN+7);
