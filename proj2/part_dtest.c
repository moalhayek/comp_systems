...
int* stats(int start, int end, int * array){
    ...
    for(int i = start;i<end; i++){
            int curr = array[i];
            //printf("Sum1 = %d, Sum2 = %d", curr, temp_sum);
            temp_sum += curr;
            //printf(", new sum = %d\n", temp_sum);
            if (curr<temp_min){
                temp_min = curr;
...
int * collate(int * array1, int *array2){
    ...
    int temp_min;
    int temp_max;
    //printf("Sum1 = %d, Sum2 = %d, new sum = %d\n", array1[0], array2[0], temp_sum);
    static int output[3]; 
...
int main(int argc, char *argv[]){
    ...
    //open up files
    int kids = 3;
    for (int file_size = 1; file_size<=1; file_size++){
